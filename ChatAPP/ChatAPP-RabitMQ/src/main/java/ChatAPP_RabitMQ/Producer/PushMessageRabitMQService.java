...
public class PushMessageRabitMQService implements RabitMQMessageProducerInterface {
	...
	public void PushSentChatMessage(MessageDTO message, long UserRecipientId) {
		   ...
		   MessagePostProcessor messagePostProcessor = RBMQMessage -> {
			   ...
			   messageProperties.setHeader(this.rabitMQProperties.getMessagePropertiesWebSocketEndPointName(), this.threadLocalWebSocketSession.getCurrentProcessWebSocketDestination());
			   messageProperties.setType(this.threadLocalWebSocketSession.getType());
